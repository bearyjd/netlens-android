...
Guard it with a composition smoke test — see "Testing". Seven screens have them (wifi, lanscan, dns, monitor, ping, portscan, traceroute). `DevicesScreen` and `HomeScreen` do not: their lists are inline in composables that take `hiltViewModel()`, so rendering them needs the list extracted into a stateless composable first.

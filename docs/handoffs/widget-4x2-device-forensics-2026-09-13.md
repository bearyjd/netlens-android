...
**Implemented and device-validated at 341×317dp:** WAN and LAN are stacked as full-width blocks — the 2x1's pattern — instead of side by side. Side-by-side caps each address at ~150dp of width, which caps type at 18sp. The Pixel 9 Pro Fold launcher cannot allocate the declared theoretical 250dp responsive bucket: its observed 2×2 minimum is 341×317dp, leaving ~325dp after horizontal padding. That fits the ~246dp 15-character IPv4 estimate at 28sp, filling the height with payload instead of padding and making all four widgets share one idiom. The verifier found both address nodes, both labels, and the Portal chip with no ellipsized text. Another launcher delivering a true 250dp width would leave ~234dp of interior, could ellipsize, and remains unverified.

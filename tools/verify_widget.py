...
Usage:  verify_widget.py <adb-serial> [--density 2.4375]
...
def classify(texts, width_dp):
    ...
    """
    tall = any(x.startswith(("Scanned", "Stale", "Not scanned", "Scanning")) for x in texts)
    if width_dp >= 300:
...
def main():
    ...
    picked = [(r, g) for r, g in groups
              if classify([n.attrib.get("text", "") for n in g if n.attrib.get("text")],
                          round((r[2] - r[0]) / density)) == want]
    if not picked:
        seen = [classify([n.attrib.get("text", "") for n in g if n.attrib.get("text")],
                         round((r[2] - r[0]) / density)) for r, g in groups]
